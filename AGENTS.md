...
- Complete super-admin setup on the dry-run VM: set `MASTER_SETUP_KEY` + `SUPER_ADMIN_EMAIL` in `.env.vps`, point `ZENOVA_LICENSE_SERVER` at a real license server, run the `/api/v1/installer/initialize-super-admin` flow, then validate role-based dashboards (see `docs/DRY_RUN_CHECKLIST.md` §3 feature checks)
- Update `docs/DRY_RUN_CHECKLIST.md` to document the installer-based super-admin creation (checklist currently assumes pre-seeded admin@zenova.app)
