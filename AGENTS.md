...
- **Super-admin setup complete on dry-run VM** (verified 2026-08-08): `MASTER_SETUP_KEY` set in `.env.vps`/backend env, `SUPER_ADMIN` license `SAL-A002-1F7A-D05D` seeded, installer `initialize-super-admin` ran → `SRV-C007D1F76D2E`; MFA two-step login verified (workaround: MFA enabled directly in DB with known TOTP secret — see MFA gap below); super-admin APIs 200 (`/platform/admin/dashboard`, `/schools`, `/licenses`). Two new migrations applied on VM (head `9a4b5c6d7e8f`): `3f5a9c1d2e4b` (license enum values) + `9a4b5c6d7e8f` (schema drift fix).
- **Role dashboards validated on dry-run VM** (2026-08-09): all 13 roles return 200/200 for `/dashboard/overview` + `/trends`; parent-portal bug found & fixed (`67a8e6e`, see Completed). Remaining §3 feature checks: corporate dashboard test account, and full CRUD feature checks (student create, attendance, report cards, payments, announcements, NFC/QR, password recovery).
