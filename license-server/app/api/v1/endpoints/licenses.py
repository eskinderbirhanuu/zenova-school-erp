...
def verify(data: LicenseVerifyRequest, db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    """Admin-protected verify endpoint — used by control center."""
...
@router.post("/school-verify", response_model=LicenseVerifyResponse)
def school_verify(data: LicenseVerifyRequest, db: Session = Depends(get_db)):
